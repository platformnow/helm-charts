...
Landscape Frontend Helm Chart for Kubernetes is a collection of Kubernetes resources to deploy Landscape IDP Frontend.
...
To install the chart with the release name `landscape-frontend`:

```console

```console
helm repo add platformnow https://platformnow.github.io/helm-charts
helm install myapp platformnow/landscape-frontend
```

## Local Builds and Install

If you already have the chart locally in a directory as **landscape-frontend**

The following command will install a default configuration into the default namespace.

```console
helm install landscape-frontend ./landscape-frontend
```

## Uninstalling the Chart

To uninstall/delete the `landscape-frontend` deployment:

```console
helm delete landscape-frontend
```
